...
public class DownloadSnapshotHandler extends HttpServlet {
  ...
  public void doGet(
    ...
    if (user == null) {
      ...
      res.getWriter().write("User not authenticated.  Please login to EveKit and try again.");
    ...
    if (acctid == -1) {
      res.getWriter().write("Account ID garbled.  Please refresh and try again.");
      res.setStatus(HttpServletResponse.SC_NOT_FOUND);
      return;
    }
    SynchronizedEveAccount acct = SynchronizedEveAccount.getSynchronizedAccount((EveKitUserAccount) user, acctid, true);
    if (acct == null) {
      res.getWriter().write("Specified account not found.  If you believe this is an error, please refresh and try again.");
      res.setStatus(HttpServletResponse.SC_NOT_FOUND);
      return;
    }
    if (!acct.getUserAccount().getUid().equals(user.getUid())) {
      res.getWriter().write("Authenticated user is not associated with this account.  Please verify you are logged into EveKit and try again.");
      res.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
      return;
    }
    ...
    try {
      ...
      res.getWriter().write("Error retrieving snapshot file.  Please contact the EveKit administrator if this error continues to occur.");
      res.setStatus(HttpServletResponse.SC_NOT_FOUND);
      return;
    }
  }
